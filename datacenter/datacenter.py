...
def update():
    ...
    if "datacenter" not in db.list_collection_names():
        collection = db.create_collection("datacenter")
        collection.create_index([("cidr", "ascending")])
    else:
        collection = db["datacenter"]
    ...
    try:
        ...
        response = client.chat_postMessage(
            channel="C05M5TZ67J6",
            text=f'Datacenter-DB-Insertion for {start_time} started \n Total-Docs:{num_docs}\n Datacenter-DB-Insertion ended! {end_time}\n Total Docs: {num_docs_end}'
        )
